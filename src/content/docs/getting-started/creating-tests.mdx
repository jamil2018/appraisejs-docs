...
- **Modules and environments** -> exist before you create the case
- **Test case form** -> captures the title, description, suite, tags, and module
- **Flow builder** -> defines what happens
- **Node addition sidebar** -> adds steps and can create needed locators
...
4. Before building the flow, fill in the basics:
   ...
   - **Module** -> select `Authentication`
   - **Test Suite** -> select an existing suite or create `Auth Smoke` from the form
...
- The suite, module, tag, and environment names are clear enough that another team member could reuse them later.
