...
<MediaPlaceholder
  type="screenshot"
  title="Sync dashboard with pending counts, `Sync All`, and tile-level actions"
  why="What recovery screen should I open when the UI and generated files look out of date?"
  priority="must-have"
  context="Troubleshooting -> Stale data recovery"
  capture="Capture `Settings -> Sync` with pending-count badges, the `Sync All` action, and individual sync tiles visible in one stable view."
  successState="A reader can identify `Sync All`, understand that tile-level actions exist, and recognize the page as the correct stale-data recovery screen."
/>

If the screen does not look like this once captured, the user should follow the CLI recovery path instead of assuming they are already on the right page.
...
Return to the page you were following in the starter path, or open [Sync and Scripts](/reference/operations/sync-and-scripts/) if you need the advanced maintenance workflow instead of the beginner-friendly fix.
